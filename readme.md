# 前言
    ...
    所有主要功能都以完成，之后工作就是优化旧代码，以及填充游戏内容。

# 最近更新

    1.实体的处理以及实体的组件处理被迁移到组件系统中，不再由实体的节点视图负责，节点视图只负责怎么渲染实体和计算实体的移动位置
    2.给玩家添加了理应加载的所有实体uuid缓存,只在集合变化时同步,解决了服务端实体删除后，客户端依然存在的问题,现在客户端会自动删除服务端传来的缓存内不存在的实体。
    3.优化整理了部分代码,更方便浏览。    
