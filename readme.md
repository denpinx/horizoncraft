...
|功能|相关类|描述|
|----|----|----|
|异步区块加载和卸载|[ChunkManageSql](script/WorldControl/ChunkManageSql.cs)|用了压缩,11kb个区块才2.2mb|
|实体生成和保存|[ChunkManageSql](script/Features/EntityManage.cs)| 跟随区块卸载和加载|
...
### 创建地下群系:
    public class MyDeepBiome : Biome
    {
        public MyDeepBiome()
        {
            ...
            //rand 随机数生成器
            //structs 存储结构集合
            GeneratorStruct = (fastnoiselite,rand,structs,gx,gy)=>{
                BlockStruct blockStruct = new BlockStruct();//结构集合
                SetBlockWork sbw = blockStruct.work;        //单个结构方块集合
                //
                sbw.ExclList.Add(
                    new(
                        //全局坐标
